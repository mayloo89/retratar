...
// TestNewTokenDoesNotRepeat would catch a token derived from the clock, a
// counter, or a source that was never seeded — the ways a token generator
// usually fails, all of which look fine in a single-call test.
...
func TestPlausibleToken(t *testing.T) {
	...
	tests := []struct {
		...
		// Nothing here claims the value is a real token; only that it is worth
		// one indexed lookup to find out.
